...
@Component({
  ...
  styleUrl: './products-report.component.css'
})
export class ProductsReportModal {
  ...
  _customReport(): void {
    ...
    this._reportService.getProductsReport(start, end).subscribe({
      next: (res) => {
        // Generate PDF Logic
        this._printService.productsReport(res);
      },
      error: (err) => {
        const errorMessage =

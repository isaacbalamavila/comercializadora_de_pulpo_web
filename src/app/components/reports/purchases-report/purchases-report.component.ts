...
@Component({
  ...
  styleUrl: './purchases-report.component.css'
})
export class PurchasesReportModal {
  ...
  _customReport(): void {
    ...
    this._reportService.getPurchasesReport(start, end).subscribe({
      next: (res) => {
        // Generate PDF Logic
        this._printService.purchasesReport(res);
      },
      error: (err) => {
        const errorMessage =
